...
module.exports = function GatheringMarkers(mod) {
    ...
    mod.hook('S_SPAWN_COLLECTION', 4, (event) => {
        ...
        if (message) mod.command.message('Found ' + event.id)
	...
	function configInit() {
        if (config) {
            ({enabled,markenabled,message,alerts,Item_ID,whiteList,markList} = config)
        } else {
    ...
    mod.command.add('gather', (p1)=> {
        ...
        if (p1 == null) {
			...
			message = !message;
			mod.command.message(message ? 'Proxy messages enabled' : 'Proxy messages disabled');
            return;
